...
#> Create 9000 new random allocation values (seed = 3751529)
...
#> Get next random result (rav:0.0788929697591811)...
#> Randomize using PBR...
#> Get block...
#> Create new block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:1, B:0], block-wise-levels[A:1/4, B:0/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0788929697591811]]
#> Get next random result (rav:0.0214472699444741)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:2, B:0], block-wise-levels[A:2/4, B:0/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0214472699444741]]
#> Get next random result (rav:0.734403318259865)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:B, overall-levels[A:2, B:1], block-wise-levels[A:2/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.734403318259865]]
#> Get next random result (rav:0.397318777628243)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:3, B:1], block-wise-levels[A:3/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.397318777628243]]
#> Get next random result (rav:0.671995327342302)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:B, overall-levels[A:3, B:2], block-wise-levels[A:3/4, B:2/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.671995327342302]]
#> Get next random result (rav:0.645773652940989)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:B, overall-levels[A:3, B:3], block-wise-levels[A:3/4, B:3/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.645773652940989]]
#> Get next random result (rav:0.0209502822253853)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:4, B:3], block-wise-levels[A:4/4, B:3/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0209502822253853]]
#> Get next random result (rav:0.122103085974231)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0, B = 1
#> Init ranges via probabilities A = 0, B = 1
#> Random result: random-result[treatment:B, overall-levels[A:4, B:4], block-wise-levels[A:4/4, B:4/4], range-set[A=[0,0], B=[0,1]; rav=0.122103085974231]]
...
|:--------------|--------------:|:--------------|:-----------|-----------------:|-----------------:|:--------------------|:--------------------|:--------------------------------------------------------------|:-------------------------------------|
| Example Trial |             1 | A             | RANDOMIZED |                1 |                0 | A:1/4               | B:0/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0788929697591811\] | bb7d741a-fbd8-41cb-98bc-67cdee7b2e33 |
| Example Trial |             2 | A             | RANDOMIZED |                2 |                0 | A:2/4               | B:0/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0214472699444741\] | 7e0a42fe-54c9-4fa0-8238-77294fb7aded |
| Example Trial |             3 | B             | RANDOMIZED |                2 |                1 | A:2/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.734403318259865\]  | ef3bc4b5-82e7-4b00-aae6-476da295a0e3 |
| Example Trial |             4 | A             | RANDOMIZED |                3 |                1 | A:3/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.397318777628243\]  | 1bbe4874-d0ba-402c-9295-c44cc7939f1b |
| Example Trial |             5 | B             | RANDOMIZED |                3 |                2 | A:3/4               | B:2/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.671995327342302\]  | 9c29027e-f514-419d-8ab0-fd6dfdea12ba |
| Example Trial |             6 | B             | RANDOMIZED |                3 |                3 | A:3/4               | B:3/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.645773652940989\]  | 622a931b-1a75-452d-be08-009474a069e4 |
| Example Trial |             7 | A             | RANDOMIZED |                4 |                3 | A:4/4               | B:3/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0209502822253853\] | 08bb4263-7a3b-401a-8c94-4bccc978e7da |
| Example Trial |             8 | B             | RANDOMIZED |                4 |                4 | A:4/4               | B:4/4               | range-set\[A=\[0,0\], B=\[0,1\]; rav=0.122103085974231\]      | 00aa6a4d-fcbe-41df-adb7-329c8d1fcffb |
