...
#> Create 9000 new random allocation values (seed = 3673778)
...
#> Get next random result (rav:0.0474661537446082)...
#> Randomize using PBR...
#> Get block...
#> Create new block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:1, B:0], block-wise-levels[A:1/4, B:0/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0474661537446082]]
#> Get next random result (rav:0.887558754300699)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:B, overall-levels[A:1, B:1], block-wise-levels[A:1/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.887558754300699]]
#> Get next random result (rav:0.0378529627341777)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:2, B:1], block-wise-levels[A:2/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0378529627341777]]
#> Get next random result (rav:0.025740688899532)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:3, B:1], block-wise-levels[A:3/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.025740688899532]]
#> Get next random result (rav:0.0272954043466598)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Init ranges via probabilities A = 0.5, B = 0.5
#> Random result: random-result[treatment:A, overall-levels[A:4, B:1], block-wise-levels[A:4/4, B:1/4], range-set[A=[0,0.5], B=[0.5,1]; rav=0.0272954043466598]]
#> Get next random result (rav:0.651880212128162)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0, B = 1
#> Init ranges via probabilities A = 0, B = 1
#> Random result: random-result[treatment:B, overall-levels[A:4, B:2], block-wise-levels[A:4/4, B:2/4], range-set[A=[0,0], B=[0,1]; rav=0.651880212128162]]
#> Get next random result (rav:0.0964265163056552)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0, B = 1
#> Init ranges via probabilities A = 0, B = 1
#> Random result: random-result[treatment:B, overall-levels[A:4, B:3], block-wise-levels[A:4/4, B:3/4], range-set[A=[0,0], B=[0,1]; rav=0.0964265163056552]]
#> Get next random result (rav:0.903147663222626)...
#> Randomize using PBR...
#> Get block...
#> Use existing block...
#> Init ranges via probabilities A = 0, B = 1
#> Init ranges via probabilities A = 0, B = 1
#> Random result: random-result[treatment:B, overall-levels[A:4, B:4], block-wise-levels[A:4/4, B:4/4], range-set[A=[0,0], B=[0,1]; rav=0.903147663222626]]
...
|:--------------|--------------:|:--------------|:-----------|-----------------:|-----------------:|:--------------------|:--------------------|:--------------------------------------------------------------|:-------------------------------------|
| Example Trial |             1 | A             | RANDOMIZED |                1 |                0 | A:1/4               | B:0/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0474661537446082\] | 96c0f6f5-5627-4555-80b8-d554da9bdafc |
| Example Trial |             2 | B             | RANDOMIZED |                1 |                1 | A:1/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.887558754300699\]  | 2ee52e5e-10e9-4364-8bba-c6f5efb1896f |
| Example Trial |             3 | A             | RANDOMIZED |                2 |                1 | A:2/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0378529627341777\] | ed21b134-0b66-4bb5-a407-169765a535fb |
| Example Trial |             4 | A             | RANDOMIZED |                3 |                1 | A:3/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.025740688899532\]  | ecb28573-b36d-449f-baab-cfbbe98b64f1 |
| Example Trial |             5 | A             | RANDOMIZED |                4 |                1 | A:4/4               | B:1/4               | range-set\[A=\[0,0.5\], B=\[0.5,1\]; rav=0.0272954043466598\] | 64953296-ebe1-411a-af36-22b3f5994647 |
| Example Trial |             6 | B             | RANDOMIZED |                4 |                2 | A:4/4               | B:2/4               | range-set\[A=\[0,0\], B=\[0,1\]; rav=0.651880212128162\]      | 152d0bc5-7510-4844-ad71-e0f4c9e8d1fd |
| Example Trial |             7 | B             | RANDOMIZED |                4 |                3 | A:4/4               | B:3/4               | range-set\[A=\[0,0\], B=\[0,1\]; rav=0.0964265163056552\]     | f96496e1-ea10-44e3-bca6-2fa7e28a96bd |
| Example Trial |             8 | B             | RANDOMIZED |                4 |                4 | A:4/4               | B:4/4               | range-set\[A=\[0,0\], B=\[0,1\]; rav=0.903147663222626\]      | 1d90f245-4fde-4f93-9503-6a6a7d7fd54f |
